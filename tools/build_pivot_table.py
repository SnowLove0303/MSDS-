...
def build(in_dir: str | Path, out_path: str | Path) -> None:
    in_dir = Path(in_dir)
    files = sorted(p for p in in_dir.glob("*.docx")
                   if not p.name.startswith("~$"))
    if not files:
        raise SystemExit(f"未找到 docx: {in_dir}")
    ...
    # ---- 数据行 ----
    for ri, (p, r) in enumerate(zip(files, results), start=3):
    ...
    print(f"  文件数: {len(files)}  列数(含型号): {col-1}  行数: {len(files)+2}")
    print(f"  节数: {len(columns)}")
