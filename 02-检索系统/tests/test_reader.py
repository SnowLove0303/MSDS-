...
PU1034 = Path(__file__).resolve().parent / "fixtures" / "PU-1034 msds_CN 国彩.docx"
...
def test_s3_real_multi_component_file():
    ...
    p = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文\冠志 guanzhi\PA-3615 MSDS（冠志）.docx")
    if not p.exists():
        return  # 文件缺失时跳过 (环境无关)
    r = read_msds(p)
    s3 = r.section(3)
...
def test_section1_letter_prefix_real_file():
    ...
    p = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文\国彩 guocai\BL-8085 msds_CN 国彩.docx")
    if not p.exists():
        return
    r = read_msds(p)
...
def test_no_empty_value_warning():
    ...
    s1 = r.section(1)
    fm = s1.field_map()
    # 调优定稿模板 (2026-08-18 改版) S1 标签为含空格 '1.1 产品名称' / '1.3 供应商信息'
    assert "1.3 供应商信息" in fm and not fm["1.3 供应商信息"], "S1 供应商信息 模板留空"
    assert "1.1 产品名称" in fm and not fm["1.1 产品名称"], "S1 产品名称 模板留空"
...
def test_component_header_saved_actual():
    ...
    from core.docx_reader import read_msds
    from core.extract import extract_doc, search_fields
    p = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\英文\国彩 guocai\PA-4771 msds_EN Guocai.docx")
    if not p.exists():
        return
    r = read_msds(p)
    sec3 = r.section(3)
...
def test_flat_component_real_pa4757():
    ...
    from core.docx_reader import read_msds
    p = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\英文\国彩 guocai\PA-4757 msds_EN 国彩.docx")
    if not p.exists():
        return
    r = read_msds(p)
    sec3 = r.section(3)
...
def test_flat_component_real_pa4408():
    ...
    from core.docx_reader import read_msds
    p = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文\国彩 guocai\PA-4408 msds_CN 国彩.docx")
    if not p.exists():
        return
    r = read_msds(p)
    sec3 = r.section(3)
...
def test_bold_english_label_field():
    ...
    from core.docx_reader import read_msds
    p = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\英文\国彩 guocai\RU-10130 msds_EN Guocai.docx")
    if not p.exists():
        return
    r = read_msds(p)
    found = False
...
def test_english_cas_placeholder_kept():
    ...
    from core.docx_reader import read_msds
    base = Path(r"F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS")
