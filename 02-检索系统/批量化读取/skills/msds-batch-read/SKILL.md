...
- 批量读取: `F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\批量化读取\batch_read.py`
- 数据库构建: `F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\批量化读取\build_db.py`
- 全量导出 Excel: `F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\批量化读取\export_excel.py`
- 封装: `F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\批量化读取\batch-read.ps1`
- 核心依赖: `F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\core\` (docx_reader / extract / structure)
...
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --sections 3 --tsv --out s3.tsv --summary

# 3. 通配符 + 关键词检索 (标签含"供应商")
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\英文\*.docx" --query "供应商" --scope label

# 4. 全库扫描 → JSON 报告 + 汇总 (不输出逐文件正文)
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --report scan.json --summary --quiet

# 5. 全库 token 完整性校验 (慢, 逐文件与原文比对)
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --verify --report integrity.json

# 6. 批量生成"文档 × 字段"对比矩阵 (宽表, 列头=Section{n} 序号 标题) → Excel
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --matrix --out 对比表.xlsx

# 7. 仅 S3 成分矩阵 (成分名/CAS/含量拆三列) → TSV
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --sections 3 --matrix --out 成分矩阵.tsv

# 8. 矩阵 + 值状态三态列 (有值/无数据/无此字段) → Excel
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --matrix --states --out 对比表_带状态.xlsx

# 9. 成分分列输出 (每文档一行, 成分1|CAS1|含量1|成分2|... 交替平铺) → TSV
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --sections 3 --comp-cols --tsv --out 成分分列.tsv

# 10. 成分分列 JSON (数据库入库用: {文件名: [{name, cas, conc}]})
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --comp-cols --json --out 成分分列.json
...
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --query "二丙二醇" --hits --summary

# 13. 只查某批次型号 + 检索 (BL/OS 型号里查"供应商"标签) → TSV
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --query "供应商" --scope label --name-filter BL,OS --tsv

# 14. 多关键词 OR (命中任一词即算; 默认 AND 全词命中)
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --query "危险 警示" --any --hits

# 15. 按节号精确检索 (scope=section 精确匹配, 等价于按节筛)
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --query 9 --scope section --tsv --out s9.tsv
```
...
```powershell
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --sections 3 --tsv --out 成分表.tsv --summary
```
...
```powershell
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\英文" --sections 1 --query "Product name" --scope label --tsv --out names.tsv
```

### 任务 4: 完整性审计 (token 级, 确认无内容遗漏)
```powershell
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --verify --report integrity.json --summary --quiet
```
...
```powershell
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS\中文" --matrix --out 对比表.xlsx --summary
```
...
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --comp-cols --json --query "二丙二醇"
```
...
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --query "供应商" --scope label \
    --name-filter BL,OS --tsv --out bl_os_供应商.tsv

# 3. 多关键词 OR (成分检索: 含"二丙二醇"或"乙二醇丁醚"任一)
.\batch-read.ps1 "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" --query "二丙二醇 乙二醇丁醚" --any --hits
...
python "F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\批量化读取\build_db.py" \
  "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\MSDS" -o MSDS数据库.sqlite
...
python "F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\tools\build_msds_db.py" `
  "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\正式库\Data Base\msds_standard.db" --init

# 2. 批量入库: 目录/多文件 → 全部按冻结骨架归一化入库
python "F:\正式项目与模块化内容\冠志\MSDS\02-检索系统\tools\build_msds_db.py" `
  "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\正式库\Data Base\msds_standard.db" `
  "F:\正式项目与模块化内容\冠志\MSDS\03-数据库\正式库\入库word  第一批"
