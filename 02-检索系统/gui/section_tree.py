# -*- coding: utf-8 -*-
"""17 节结构化显示: 树形 + 与固定 Word 模板对齐的节内容视图。"""
...
from core.edit_policy import is_editable_component, is_editable_row

_ENGINE_PROGRAM_DIR = Path(__file__).resolve().parents[2] / "04-推断引擎" / "推断引擎程序"
if str(_ENGINE_PROGRAM_DIR) not in sys.path:
    sys.path.insert(0, str(_ENGINE_PROGRAM_DIR))
from pictogram_registry import display_items, extract_codes  # noqa: E402
...
class SectionTree(ttk.Frame):
    ...
    def _build(self):
        ...
        self.tree.heading("#0", text="MSDS 17 节（S0~S16）")
...
class SectionView(ttk.Frame):
    ...
    def _render_row(self, row):
        ...
        # 固定模板权限由白名单决定；不再把 Word 解析器的启发式 editable
        # 或用户手动徽章状态当作授权来源。
        editable = is_editable_row(self._num, row)
        if row.span:
    ...
    def _three_col_row(self, key, seq, label, value, editable: bool):
        ...
        if "[象形图" in (value or "") or codes:
            v = self._value_cell_with_images(value, bg, val_fg, wrap=560, codes=codes)
            if v is not None:
    ...
    def _value_cell_with_images(self, value, row_bg, fg, wrap=None, codes=None):
        ...
        # 新链路按 GHS 代码取唯一标准红菱形图片；旧源文件的 [象形图] 占位
        # 才继续按原文档图片顺序读取，避免把原始黑色图标误当成 GHS 图。
        canonical_items = display_items(value) if codes else []
        if canonical_items:
            take = [(blob, ext, code) for code, _label, blob, ext in canonical_items]
        else:
            sec_imgs = [im for im in (self._result.images if self._result else [])
                        if im.section == self._num]
            take = [(im.blob, im.ext, "") for im in sec_imgs[self._img_index:self._img_index + n]]
            self._img_index += n
        ...
        def factory(cell):
            ...
            for blob, ext, code in take:
                ph = self._photo_of(blob, ext)
                if ph is None:
                    continue
                ...
                il.grid(row=0, column=col, sticky="nw", padx=(6 if col else 6, 2),
                        pady=7)
                il.bind("<Button-1>",
                        lambda e, b=blob, x=ext: self._save_image(b, x))
                col += 1
    ...
    def _component_table(self, sec):
        ...
        for ci, comp in enumerate(sec.components):
            ...
            editable = is_editable_component(self._num, comp)
            bg = COLOR_ROW_ALT if (self._table_ri % 2 == 0) else COLOR_PANEL
