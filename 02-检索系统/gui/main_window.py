...
class MainWindow(tk.Tk):
    ...
    def _build_toolbar(self):
        ...
        # ---- 行1: 文件与导出 ----
        tk.Label(bar1, text="🔒 固定模板已锁定", bg="#E8EAED", fg=COLOR_TEXT,
                 padx=12, pady=3, font=("Microsoft YaHei", 10, "bold")).pack(side="left", padx=(0, 8))
        ...
        # ---- 行2: 工具 / 检索 / 状态 ----
        btn(bar2, "📝 17节覆写表单", self._open_form, bg=COLOR_GREEN)
        btn(bar2, "🔎 推断引擎", self._open_inference, bg=COLOR_ACCENT)
        ...
        for num, sec in src.sections.items():
            ...
            for row in sec.iter_rows():
                ...
                rows.append({
                    ...
                    "label": row.label,
                    "value": row.value,
                    "editable": is_editable_row(num, row),
                })
            data["sections"][str(num)] = {
                ...
                "components": [
                    {"name": c.name, "cas": c.cas, "conc": c.conc,
                     "editable": is_editable_component(num, c)}
    ...
    def _open_form(self):
        """打开固定模板 17 节覆写表单并产出 Word。"""
        try:
            from .overwrite_form_window import OverwriteFormWindow
            OverwriteFormWindow(self, template_path=TEMPLATE_PATH)
        except Exception as exc:
            messagebox.showerror("17节覆写表单启动失败", str(exc))

    def _open_inference(self):
        """打开 S1/S3/S9 → Section 2 推断工作台。"""
        try:
            from .inference_window import InferenceWindow
            InferenceWindow(self)
        except Exception as exc:
            messagebox.showerror("推断引擎启动失败", str(exc))
